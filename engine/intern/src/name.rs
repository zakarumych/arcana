...
use crate::{Ident, intern::INTERNER};

#[macro_export]
macro_rules! name {
    ($($tokens:tt)+) => {{
        const { $crate::assert_ascii_name(stringify!($($tokens)+)); }
        $crate::Name::from_name_static(stringify!($($tokens)+))
    }};
}
...
impl<'de> serde::de::Visitor<'de> for NameVisitor {
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str("Expected non-empty unicode string without control characters")
    }
...
/// Returns error if it is not.
#[inline(never)]
pub const fn assert_ascii_name(s: &str) {
    ...
    loop {
        match bytes {
            ...
            [byte, tail @ ..] => {
                bytes = tail;
                assert!(!byte.is_ascii_control(), "Bad character");
            }
        }
    }
}
