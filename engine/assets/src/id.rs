...
/// A 64-bit value used to identify an asset.
///
/// These values are generated by the engine instance using combination of
/// timestamp, sub-second counter and random number.
///
/// A timestamp with second precision is used to make sure that identical IDs cannot be generated
/// even if random number would be repeated after process restarts.
///
/// If IDs are generated faster than one per second, timestamp would be bumped to the next value.
