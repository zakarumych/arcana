...
pub trait InputFilter: 'static {
    ...
    fn filter(&mut self, world: &mut World, event: &Input) -> bool;
}
...
{
    #[inline(always)]
    fn filter(&mut self, _world: &mut World, event: &Input) -> bool {
        self.0(event)
    }
}
...
impl InputFunnel {
    ...
    #[inline]
    pub fn filter(&mut self, world: &mut World, event: &Input) -> bool {
        for filter in self.filters.iter_mut() {
            if filter.filter(world, event) {
                return true;
            }
        }
        false
