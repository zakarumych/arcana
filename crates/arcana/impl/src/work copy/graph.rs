...
impl WorkGraph {
    ...
    pub fn insert(&mut self, job: impl JobDesc) -> JobId {
    ...
    pub fn run(
        ...
        for &job in self.queue.iter().rev() {
            ...
            let node = self.nodes.get_mut(&job).unwrap();
        ...
        let cbufs = self.cbufs.drain().map(|e| e.finish().unwrap());

        queue.submit(cbufs, true);
    }
}
