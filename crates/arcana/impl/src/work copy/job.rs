...
impl Setup<'_> {
    ...
    pub fn build(self, name: String) -> JobNode {
        JobNode {
            name,
            inputs: self.inputs,
...
pub trait Job: 'static {
    ...
    /// Called each frame at execution stage.
    fn run(&mut self, runner: Runner<'_>);
}

impl<P, R> Job for (P, R)
where
    P: FnMut(Planner<'_>) + 'static,
    R: FnMut(Runner<'_>) + 'static,
{
    fn plan(&mut self, planner: Planner<'_>) {
        (self.0)(planner)
    }
