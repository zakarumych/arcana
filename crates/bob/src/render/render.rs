...
impl fmt::Debug for TargetId<nix::Image> {
    #[inline(always)]
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("TargetId<nix::Image>")
            .field(&self.0)
            .field(&self.1)
            .finish()
    }
...
impl fmt::Debug for TargetId<nix::Buffer> {
    #[inline(always)]
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("TargetId<nix::Buffer>")
            .field(&self.0)
            .field(&self.1)
            .finish()
    }
...
impl<'a> RenderBuilderContext<'a> {
    ...
    pub fn create_target<T>(&mut self, name: &str, stages: nix::PipelineStages) -> TargetId<T>
    where
        T: RenderTargetType,
    {
        let target = self
    ...
    pub fn read_target<T>(&mut self, target: TargetId<T>, stages: nix::PipelineStages)
    where
        T: RenderTargetType,
    {
        self.graph
