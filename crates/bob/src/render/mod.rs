...
pub trait RenderTargetType: 'static {
    ...
    fn write_target<'a>(
        ...
        id: TargetId<Self>,
        encoder: &mut nix::CommandEncoder,
    ) -> &'a Self;
...
impl RenderTargetType for nix::Image {
    fn add_target(
        graph: &mut RenderGraph,
        name: Box<str>,
        target_for: RenderId,
    ...
    fn get_target(graph: &RenderGraph, id: NonZeroU64) -> &RenderTarget<nix::Image> {
        graph.image_targets.get(&id).expect("Invalid target id")
    }

    fn get_target_mut(graph: &mut RenderGraph, id: NonZeroU64) -> &mut RenderTarget<nix::Image> {
        graph.image_targets.get_mut(&id).expect("Invalid target id")
    }

    fn write_target<'a>(
        cx: &'a mut RenderContext<'_, '_>,
        id: TargetId<nix::Image>,
        encoder: &mut nix::CommandEncoder,
    ) -> &'a nix::Image {
    ...
    fn add_dependency(edges: &mut RenderNodeEdges, id: TargetId<nix::Image>) {
        edges.depends_on_images.insert(id);
    }

    fn add_renders_to(edges: &mut RenderNodeEdges, id: TargetId<nix::Image>) {
        edges.renders_to_images.insert(id);
    }

    fn depends_on(edges: &RenderNodeEdges) -> hashbrown::hash_set::Iter<'_, TargetId<nix::Image>> {
        edges.depends_on_images.iter()
    }

    fn renders_to(edges: &RenderNodeEdges) -> hashbrown::hash_set::Iter<'_, TargetId<nix::Image>> {
        edges.renders_to_images.iter()
    }
}

impl RenderTargetType for nix::Buffer {
    fn add_target(
        graph: &mut RenderGraph,
        name: Box<str>,
        target_for: RenderId,
    ...
    fn get_target(graph: &RenderGraph, id: NonZeroU64) -> &RenderTarget<nix::Buffer> {
        graph.buffer_targets.get(&id).expect("Invalid target id")
    }

    fn get_target_mut(graph: &mut RenderGraph, id: NonZeroU64) -> &mut RenderTarget<nix::Buffer> {
        graph
            .buffer_targets
            .get_mut(&id)
    ...
    fn add_dependency(edges: &mut RenderNodeEdges, id: TargetId<nix::Buffer>) {
        edges.depends_on_buffers.insert(id);
    }

    fn add_renders_to(edges: &mut RenderNodeEdges, id: TargetId<nix::Buffer>) {
        edges.renders_to_buffers.insert(id);
    }

    fn depends_on(edges: &RenderNodeEdges) -> hashbrown::hash_set::Iter<'_, TargetId<nix::Buffer>> {
        edges.depends_on_buffers.iter()
    }

    fn renders_to(edges: &RenderNodeEdges) -> hashbrown::hash_set::Iter<'_, TargetId<nix::Buffer>> {
...
pub struct RenderGraph {
    ...
    image_targets: HashMap<NonZeroU64, RenderTarget<nix::Image>>,
    buffer_targets: HashMap<NonZeroU64, RenderTarget<nix::Buffer>>,
    presents: HashMap<WindowId, TargetId<nix::Image>>,
    next_id: u64,
}
...
impl RenderGraph {
    ...
    pub fn present(&mut self, target: TargetId<nix::Image>, window: WindowId) {
        let rt = self
            .image_targets
            .get_mut(&target.0)
            .expect("Invalid target id");
...
pub struct UpdateTargets {
    update_images: HashSet<TargetId<nix::Image>>,
    update_buffers: HashSet<TargetId<nix::Buffer>>,
}
...
type BarrierMap<'a, T> = BlinkHashMap<'a, TargetId<T>, Range<nix::PipelineStages>>;
...
pub struct RenderContext<'a, 'b> {
    ...
    images: &'a mut TargetMap<'b, nix::Image>,

    // Maps render target to pipeline stages that need to be waited for.
    init_images: &'a mut ImageInitSet<'b>,
    write_image_barriers: &'a mut BarrierMap<'b, nix::Image>,
    read_image_barriers: &'a mut BarrierMap<'b, nix::Image>,

    // Maps render target ids to buffer resources.
    buffers: &'a mut TargetMap<'b, nix::Buffer>,

    // Maps render target to pipeline stages that need to be waited for.
    write_buffer_barriers: &'a mut BarrierMap<'b, nix::Buffer>,
    read_buffer_barriers: &'a mut BarrierMap<'b, nix::Buffer>,
...
impl<'a> RenderContext<'a, '_> {
    ...
    pub fn write_target<T>(&mut self, id: TargetId<T>, encoder: &mut nix::CommandEncoder) -> &T
    where
        T: RenderTargetType,
    {
    ...
    pub fn read_target<T>(&mut self, id: TargetId<T>, encoder: &mut nix::CommandEncoder) -> &T
    where
        T: RenderTargetType,
    {
...
pub struct RenderNodeEdges {
    depends_on_images: HashSet<TargetId<nix::Image>>,
    renders_to_images: HashSet<TargetId<nix::Image>>,
    depends_on_buffers: HashSet<TargetId<nix::Buffer>>,
    renders_to_buffers: HashSet<TargetId<nix::Buffer>>,
}
...
pub struct RenderResources {
    surfaces: HashMap<WindowId, nix::Surface>,
    images: HashMap<NonZeroU64, nix::Image>,
    buffers: HashMap<NonZeroU64, nix::Buffer>,
}
...
pub fn render_system(world: &mut World, mut state: State<RenderState>) {
    ...
    let mut owned_queue = world.get_resource_mut::<nix::Queue>();
    let mut shard_queue = world.get_resource_mut::<Arc<Mutex<nix::Queue>>>();
    let mut queue_lock;
...
pub fn render<'a>(
    ...
    while let Some(tid) = image_targets_to_update.pop() {
        let rt = graph.get_target::<nix::Image>(tid.0);
    ...
    while let Some(tid) = buffer_targets_to_update.pop() {
        let rt = graph.get_target::<nix::Buffer>(tid.0);
    ...
    while let Some(rid) = render_queue.pop_front() {
        ...
        let mut ready = render
            .edges
            .depends_on::<nix::Image>()
            .all(|tid| image_scheduled.contains(&tid));

        ready &= render
            .edges
            .depends_on::<nix::Buffer>()
        ...
        if ready {
            ...
            for tid in render.edges.renders_to::<nix::Image>() {
                let inserted = image_scheduled.insert(tid);
                debug_assert!(inserted, "Target already scheduled");
            }

            for tid in render.edges.renders_to::<nix::Buffer>() {
