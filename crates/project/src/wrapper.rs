...
// pub fn build_project(name: &str, root: &Path) -> miette::Result<Build> {
//     let workspace = root.join("workspace");
...
//         .map_err(|err| {
//             miette::miette!(
//                 "Failed to start building project '{name}' in '{}'. {err}",
//                 workspace.display()
//             )
//         })?;
...
pub fn build_plugin(name: &str, root: &Path) -> miette::Result<PluginBuild> {
    let workspace = root.join("workspace");
...
fn plugin_lib_path(name: &str, root: &Path) -> PathBuf {
    let mut lib_path = root.to_owned();
