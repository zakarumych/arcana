...
use crate::{dependency::Dependency, manifest::Plugin, path::make_relative, WORKSPACE_DIR_NAME};
...
pub fn new_plugin_crate(name: &str, path: &Path, engine: Dependency) -> miette::Result<()> {
    if path.exists() {
        miette::bail!(
